...
{
	// WebCapsule
    TRACE_EVENT0("blink", "WebCapsule::RenderMenuList::valueChanged : Total");
    /////////////
	...
	// WebCapsule
    // FIXME: It would probably be much better to instrument WebPopupMenuImpl::handleInputEvent, but it seems
    // difficult to recover the page handle for the page where the popup menu is displayed
    TRACE_EVENT_BEGIN0("blink", "WebCapsule::RenderMenuList::valueChanged : Instrumentation");
    WebCapsule::log(WebCapsule::DebugLogLevel, "RenderMenuList::valueChanged");
    InspectorInstrumentation::handleRenderMenuListValueChanged(document().page(), listIndex, fireOnChange, node());
    TRACE_EVENT_END0("blink", "WebCapsule::RenderMenuList::valueChanged : Instrumentation");
    /////////////
...
{
	WebCapsule::log(WebCapsule::DebugLogLevel, "RenderMenuList::didSetSelectedIndex (%d)", listIndex);
    didUpdateActiveOption(selectElement()->listToOptionIndex(listIndex));
}

void RenderMenuList::didUpdateActiveOption(int optionIndex)
{
	WebCapsule::log(WebCapsule::DebugLogLevel, "RenderMenuList::didUpdateActiveOption (%d)", optionIndex);
...
{
	WebCapsule::log(WebCapsule::DebugLogLevel, "RenderMenuList::selectedIndex");
    HTMLSelectElement* select = selectElement();
...
{
    WebCapsule::log(WebCapsule::DebugLogLevel, "RenderMenuList::setTextFromItem (%d)", listIndex);
